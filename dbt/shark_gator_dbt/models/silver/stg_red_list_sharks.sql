...
SELECT DISTINCT
    ...
    UPPER(T.orderName) AS ORDER_NAME,
...
FROM {{ source('bronze_source', 'RAW_SHARK_RED_LIST_ASSESSMENTS') }} A
...
JOIN {{ source('bronze_source', 'RAW_SHARK_RED_LIST_COMMON_NAMES') }} C
ON A.internalTaxonId = C.internalTaxonId
JOIN {{ source('bronze_source', 'RAW_SHARK_RED_LIST_TAXONOMY') }} T
ON T.internalTaxonId = A.internalTaxonId
WHERE T.orderName IN (
      'CARCHARHINIFORMES',
      'LAMNIFORMES',
      'ORECTOLOBOFORMES',
      'SQUALIFORMES',
